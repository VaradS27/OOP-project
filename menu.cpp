...
Menu::Menu(float width, float height){
    ...
    text[0].setPosition(sf::Vector2f((width/2), (height/(MAX_NUMBER_OF_OPTIONS+1) * 1)));

    // rules of the game
    text[1].setFont(font);
    text[1].setFillColor(sf::Color::White);
    text[1].setString("RULES");
    text[1].setPosition(sf::Vector2f((width/2), (height/(MAX_NUMBER_OF_OPTIONS+1) * 2)));

    // about the game
    text[2].setFont(font);
    text[2].setFillColor(sf::Color::White);
    text[2].setString("ABOUT");
    text[2].setPosition(sf::Vector2f((width/2), (height/(MAX_NUMBER_OF_OPTIONS+1) * 3)));
